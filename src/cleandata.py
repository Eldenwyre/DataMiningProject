...
def first_clean(infile: str, outfile: str) -> None:
    ...
    df["foreign_worker"].replace(
        to_replace="1", value="yes", inplace=True,
    )
    ...
    df["num_dependents"] = cleandata.bounding(
        df["num_dependents"],
        lower_bound=0,
        upper_bound=1000,
        replace_with="?",
...
def perform_and_display_spearman(
    df: pd.DataFrame, headers: List[str], threshold: float = 0.85
) -> None:
...
def perform_and_display_chi_square(
    df: pd.DataFrame, headers: List[str], significance: float
) -> None:
        ...
        for c1 in range(len(headers)):
            for c2 in range(c1 + 1, len(headers)):
                ...
                if chi > critical_value:
                    ...
                    f.write(
                        f"[Independent: {not (chi > critical_value)}] (pval: {pval})"
                    )
                    f.write(
                        f"Chi: {format(chi, '.2f')} cv: {format(critical_value, '.2f')}\n"
                    )
...
def main() -> None:
    ...
    IN_OUT_PAIRS = [
        ("data/credit.csv", "data/clean.csv"),
        ("data/clean.csv", "data/clean_binned.csv"),
        ("data/clean.csv", "data/clean_normalized.csv"),
    ]
    # Initial Cleans
...
def drop_features():
    ...
    df_list = [
        ...
        pd.read_csv("data/clean_binned.csv"),
        pd.read_csv("data/clean_normalized.csv"),
    ]
    ...
    for df in df_list:
        ...
        df.drop(
            ["age", "num_dependents", "residence_since", "existing_credits"],
            axis=1,
            inplace=True,
        )
        df.drop(["duration", "installment_commitment"], axis=1, inplace=True)
        # Drop works_outside_US, 1:1 with foreign worker
        df.drop(["works_outside_US"], axis=1, inplace=True)
